...
exports.down = async function(knex) {
  ...
  await knex.schema.createTable('faculties', (table) => {
    ...
    table.integer('university_id').notNullable().references('university_id').inTable('universities').onDelete('CASCADE');
    table.timestamp('created_at').defaultTo(knex.fn.now());
    table.timestamp('updated_at').defaultTo(knex.fn.now());
  ...
  await knex.schema.alterTable('study_programs', (table) => {
    ...
    table.integer('faculty_id').nullable().references('faculty_id').inTable('faculties').onDelete('CASCADE');
  });
};
